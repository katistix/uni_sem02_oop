...
void ServiceProduse::adauga(const std::string& nume, const std::string& tip, const double pret, const std::string& producator) {
    const Produs p(nextId++, nume, tip, pret, producator);
    repo.adauga(p);
}
...
void ServiceProduse::modifica(const int id, const std::string& nume, const std::string& tip, const double pret, const std::string& producator) const {
...
std::vector<Produs> ServiceProduse::filterByFunction(bool (*fn)(const Produs&)) const {
    const std::vector<Produs> produse = repo.getAll();
